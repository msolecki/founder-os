...
| `/dashboard` | Render the workspace as one page — what the bets, pipeline, week, queue and close actually say right now, every number carrying the file it came from. A run that writes at all — not `--json`, and not refused — writes `facts.json` and the day's row of `snapshots.csv` into every readable active business's `_dashboard/`, and a `.gitignore` there only when that directory has none; `--out` moves the page alone, and is refused onto a path an agent owns or a file the run maintains. No agent may write the directory and nothing may cite it as evidence. Flags, exit codes, and what the directory holds: [`dashboard.md`](dashboard.md). *(standalone — run it yourself)* |
