...
- [x] Add adapter/config validation and subprocess hook tests, including one payload that omits `agent_type` after recording a real `turn_id`.
- [x] Run focused tests and record RED failures for missing configs, missing roundtrip, and overly broad/incorrect guard decisions.
- [x] Add both host configs, manifest reference, normalized hook matching, atomic role mapping, and capability-aware guard decisions.
- [x] Extend package validation to compile/validate the MCP entry and both adapter shapes without assuming Codex native role discovery.
- [x] Re-run focused suites, installed-copy smoke, full Python/Node suites, package validation, generated-command check, plugin-creator validation, and `git diff --check`.
- [x] Commit only Task 4 files using an explicit `--only` pathspec and message `feat: wire state gateway into Claude and Codex`.
...
- [x] Add contract tests that enumerate all role files and mutate a fixture with `Agent(cfo)`, `Task`, a direct write tool, a changed fallback role body, and a malformed delegation request.
- [x] Run focused tests and record RED failures against current manager edges and file-tool allowlists.
- [x] Write the orchestration reference and migrate all 13 role contracts without changing their business decision ownership.
- [x] Replace `check_agent_graph` with fail-closed one-level orchestration validation and update validator fixtures.
- [x] Re-run focused tests, full suites, package validation, generated-command check, and `git diff --check`.
- [x] Commit the reference, all role files, validator, and tests only, using explicit paths and message `refactor: orchestrate Founder OS roles as siblings`.
...
- [x] Add failing semantic contract tests for exact ordered roles, checkpoint re-reads, session closure, carried-answer bounds, and final evaluation ownership.
- [x] Run focused tests and record RED failures from current direct invocation/delegation wording.
- [x] Migrate `founder-os-init`, `strategic-evaluation`, `situation-review`, and `kill-or-continue` to the shared delegation-request format.
- [x] Re-run focused tests, all workflow/validator tests, generated-command check, package validation, and `git diff --check`.
- [x] Commit only migrated workflows and tests with explicit paths and message `refactor: persist sibling workflow checkpoints`.
...
- [x] Add subprocess SessionStart cases and table-driven YAML-shape mutations; assert model-visible JSON, matching stderr, controlled exit behavior, and `FAIL:` output.
- [x] Run focused suites and record RED failures for silent context loss and uncaught/incorrect YAML shapes.
- [x] Implement warning output, strict frontmatter/tools validation, and the single ownership-schema loader used by all validator checks.
- [x] Re-run focused tests, installed-copy smoke, full Python suite, package validation, generated-command check, and `git diff --check`.
- [x] Commit only Task 7 files with an explicit pathspec and message `fix: surface context and YAML contract failures`.
...
- [x] Write trust-claim parity and temporary-tree link/anchor tests with literal expected diagnostics.
- [x] Run focused tests and record RED failures for the missing page and checker.
- [x] Create the accessible static Trust Center, implement the parser/checker, and wire it into CI.
- [x] Run focused tests, the real local-link command, Node behavior tests, full Python tests, package validation, and `git diff --check`.
- [x] Commit only Task 8 files with explicit paths and message `docs: ship and validate the Trust Center`.
...
- [x] Add behavior tests against real exported controller functions and DOM fakes; assert the exact visible/focused panel and absence of temporary textareas.
- [x] Run Node tests and record RED failures for hidden focus and false/exception copy paths.
- [x] Move the existing workspace and clipboard controllers into the same-origin CommonJS-compatible module and implement the minimal focus/copy fixes there.
- [x] Re-run Node tests, Python docs/link tests, package validation, and `git diff --check`.
- [x] Commit only Task 9 files with explicit paths and message `fix: preserve website focus and clipboard truth`.
...
- [x] Add failing release/count/document-contract tests that derive expected totals from agents, skills, cadence rows, validator `CHECKS`, and doctor sections.
- [x] Run focused tests and record RED failures for current 2.4.0/full-host/count/ignore drift.
- [x] Update metadata and docs, run `python3 scripts/generate_commands.py founder-os`, and preserve historical changelog text below the new section.
- [x] Re-run release/docs/validator/link tests, generated-command check, all Python/Node suites, package validation, both plugin validators, and `git diff --check`.
- [x] Commit only explicit Task 10 paths with message `docs: synchronize Founder OS 2.5.0 host parity`; verify the unrelated staged deletion remains staged.
...
- 2026-07-27: Task 4 completed in `0cf1924`. Initial RED ran 63 adapter/guard tests with 11 failures and 2 errors: the Claude adapter was absent, valid local gateway calls and the real Codex turn roundtrip were denied, while WebSearch and direct file tools remained allowed. Validator and installed-copy REDs exposed missing adapter checks and a superseded direct-owner smoke contract. Review-driven regressions then closed missing/corrupt Codex identity mappings, present-invalid identity markers, malformed tool/capability inputs, clock failure, unsafe persisted-session records, and removal of both adapters. Final independent review approved with no Critical, Important, or Minor finding. Final GREEN: 74/74 focused tests, installed-copy smoke PASS, 315/315 full Python tests, and 2/2 Node behavior tests; package validation reported 13 agents, 52 skills, 0 errors; command generation, Codex plugin validation, and `git diff --check` were clean. The unrelated staged deletion and `_Context.md` remained untouched.
- 2026-07-27: Task 5 completed in `7115a10`. Initial RED ran 14 orchestration tests with 28 failures and 19 errors against direct file tools, manager `Agent(...)` edges, missing sibling contracts, and absent validator APIs. The implementation gave all 13 roles the same bounded six-tool state gateway, moved delegation to the main thread, documented byte-identical generic fallback, and replaced graph validation with one-level fail-closed checks. Author and independent review added regression coverage for malformed and system workflows, unsafe persistence paths, unbounded handoffs, role-file traversal, hidden active dispatch prose, malformed gateway suffixes, falsey write metadata, vacuous checkpoints, and workspace/correlation substitution. Final review approved with no remaining findings. Fresh GREEN: 122/122 scoped tests, 333/333 full Python tests, 2/2 Node behavior tests, installed-copy smoke PASS, validator 13/52/0, generator current, and `git diff --check` clean. The unrelated staged deletion and `_Context.md` remained untouched.
- 2026-07-27: Task 6 completed in `328ad30`. Semantic RED ran 53 tests with 11 expected failures for the missing persisted sibling sequence, lifecycle gates, bounded requests, strategic phases, exact router output, and legacy nested wording. The four workflows now use the canonical six-field delegation request; onboarding advances through six fresh owner sessions and resumes at the first invalid checkpoint, while strategic evaluation isolates two or three attributed read-only perspectives before Board challenge and final Chief of Staff persistence. Author and independent review both approved with no remaining findings. Final GREEN: 53/53 focused tests, 340/340 full Python tests, 2/2 Node behavior tests, installed-copy smoke PASS, validator 13/52/0, generator current, and `git diff --check` clean. The unrelated staged deletion and `_Context.md` remained untouched.
- 2026-07-27: Task 7 completed in `067b2aa`. Focused RED ran 93 tests with 14 expected failures and 14 expected errors: broken installed guidance stayed invisible, non-mapping frontmatter and invalid tool values were accepted or coerced, and malformed ownership containers escaped a shared typed boundary. SessionStart now emits the same resolved-path no-advice warning to model context and stderr without crashing the host; frontmatter and tools are strict, and one validated ownership loader supplies every ownership/section consumer. Author and independent review approved with no findings. Final GREEN: 93/93 focused tests, 348/348 full Python tests, 2/2 Node behavior tests, installed-copy smoke PASS, validator 13/52/0, generator current, and `git diff --check` clean. The unrelated staged deletion and `_Context.md` remained untouched.
