...
examples are composed from `.promptscript/project.prs` and its focused
fragments. `.promptscript/agents.prs` composes one `.promptscript/agents/<name>.prs`
file per role. Every skill under `.promptscript/skills/` is auto-discovered and
compiled for the selected host. The generated files in this directory are
adapters, not a second prompt source.
