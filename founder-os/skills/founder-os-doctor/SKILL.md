...
| **Local agent overreaches** | A file in `_local/agents/` omits `tools:`, or names `Bash`, `WebFetch`, `WebSearch`, `Agent`, or an `mcp__*` tool | Omitting `tools:` inherits everything, including Bash. **The overlay cannot actually widen anything** — `check_outbound` in the guard denies subagents the outbound tools without consulting any map — so this is an early warning rather than a breach: the founder wrote an agent that will be denied mid-run, and finding out here beats finding out then. House rule 0 is not a default the overlay may override. |
...
| **Installed copy drift** | A skill in `_local/skills/` has no installed copy under `~/.claude/skills/`, or the installed copy differs from its source | Two copies of anything is a second map, and this one is deliberate — the source of truth is in the workspace, the installed copy is what the host can load. Uninstalled means a workflow that exists and never runs. Diverged means the founder edited the copy the doctor cannot vouch for. Report which of the two and hand to `/skill-forge`; **never sync them yourself** — you cannot tell which side is the intended one. |
