...
`setup-cadences` is a third kind: **standalone**. It edits the founder's local
scheduler state, which takes tools no agent may hold, so it belongs to no agent
by design — `STANDALONE_SKILLS` in `scripts/validate_package.py` is that list. A
